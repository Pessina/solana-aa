...
pub struct AbstractAccountOperation<'info> {
    ...
    #[account(
        init_if_needed,
        payer = signer,
        space = if abstract_account.to_account_info().data_len() > 0 {
            abstract_account.to_account_info().data_len()
        } else { 
            AbstractAccount::initial_size()
        },
...
pub fn create_account_impl(
    ...
    if account_id != ctx.accounts.account_manager.next_account_id {
        return Err(ErrorCode::InvalidAccountId.into());
    }

    let new_size = AbstractAccount::initial_size() + identity_with_permissions.byte_size();
